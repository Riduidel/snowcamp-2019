...
`jx create cluster gke --cluster-name=snowcamp-2019 --project-id=snowcamp-2019-223915 --min-num-nodes=5 --max-num-nodes=10 --zone=europe-west1-d --machine-type=n1-standard-2 --gitops=false --batch-mode=true --install-dependencies=true --environment-git-owner=Zenika --git-api-token=$GIT_ACCESS_TOKEN --git-username=Riduidel`
--
...
kubectl apply -f src/k8s/kafka/kafka-topics-ui-pod.yml
kubectl apply -f src/k8s/kafka/kafka-topics-ui-service.yml
...
[source]
----
kubectl port-forward --namespace kafka kafka-topics-ui 18000 8000
----
--
...
[plantuml, {plantumldir}1_assemblage, png]
----
include::plantuml/1_assemblage.plantuml[]
----
